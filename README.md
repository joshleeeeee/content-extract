...
[![Version](https://img.shields.io/badge/version-v1.8.2-blue.svg)](https://github.com/joshleeeeee/content-extract)
...
一款极致小巧、功能强大的在线内容转换与导出工具。支持将飞书/Lark 等文档（包括受保护无法复制的文档）、BOSS 直聘职位信息、京东/淘宝/天猫商品评论，以及抖音/小红书/B 站评论，一键转换为 Markdown、富文本、PDF、CSV 或 JSON，并提供更强大的批量任务大厅、进度追踪与多窗口抓取能力。
...
- **🤖 多平台适配**: 完美支持 **飞书/Lark** 文档、**BOSS 直聘** 职位信息提取（含滚动加载场景），以及 **京东/淘宝/天猫/抖音/小红书/B站** 评论抓取。
- **🛍️ 评论提取**: 支持电商评论与社媒评论双场景抓取，自动展开回复并前台滚动增量提取，输出 **Markdown/HTML/PDF/CSV/JSON**。
- **📦 批量抓取**: 自动扫描页面中的文档链接、职位卡片或商品链接，按任务类型选择导出格式后，一键排队抓取，支持多窗口平铺抓取模式。
- **🔗 智能合并**: 针对 BOSS 直聘等场景，支持将多篇内容自动合并为一份文档导出。
- **🔍 实时检测**: 插件 Popup 自动检测网页支持状态，智能切换抓取模式。
- **⚡ 下载中心**: 内置下载管理，支持打包为 ZIP、历史记录持久化与批量任务进度条展示；筛选后的全选与空状态表现更准确。
...
5. **批量抓取**:
   ...
   - 勾选目标后点击“开始抓取”，可按需启用 **多窗口模式**，将批量任务以平铺窗口方式并行执行。
   - 下载中心会显示更清晰的 **批量任务进度条**，并且筛选结果下的全选/空状态会与当前视图保持一致。
6. **高级设置 (图床/本地)**:
...
- [x] **界面与能力页升级** (v1.8.1): 更新品牌图标与弹窗界面，补齐更精确的能力说明与官网安装指引。
- [x] **批量任务大厅增强** (v1.8.2): 新增批量抓取进度条、多窗口平铺抓取模式，修复筛选视图下的全选与空状态，并补强 BOSS 直聘滚动加载场景支持。
